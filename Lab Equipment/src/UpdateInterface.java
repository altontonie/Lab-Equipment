...
public class UpdateInterface extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				UpdateInterface frame = new UpdateInterface();
	...
	public UpdateInterface() {
		...
		try {
			...
			String sql = "SELECT * FROM logindb where User_Id="+User_Id+"";
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(passwordField.getText().length()!=0 && usernameField.getText().length()!=0 && +emailField.getText().length()!=0 && lnameField.getText().length()!=0 && fnameField.getText().length()!=0) {
					try {
						...
						Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","");
						Statement stmt = conn.createStatement();
						String sql = "UPDATE logindb SET First_Name='"+fnameField.getText()+"',Last_Name='"+lnameField.getText()+"',"
								+ " Email='"+emailField.getText()+"' , Username='"+usernameField.getText()+"', Password='"+passwordField.getText().toString()+"',"
										+ " Role='"+new String("user")+"' WHERE User_Id="+User_Id+"";
